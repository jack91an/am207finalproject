function [fwbkProbsDays, fwbkNormedTransProbsDays, fwbkNormedDurProbsDays] = hhsmmFwBkDaysSplit(modelInfo, learnedParams, testFeatMatDays)
...
fwbkNormedTransProbsDays = cell(length(testFeatMatDays),1);
fwbkNormedDurProbsDays = cell(length(testFeatMatDays),1);
...
for n=1:length(testFeatMatDays),
    ...
    forwDurProbs= zeros(numAct, maxDur, numTimeSteps);    
    backVar = zeros(numAct, numTimeSteps);
    backDurProbs = zeros(numAct,  maxDur, numTimeSteps);
    fwbkProbs = zeros(numAct, numTimeSteps);
    ...
    for k=1:numTimeSteps,
        ...
        tempForward = -inf(numAct,min(t,maxDur));
        for d=1:min(k,maxDur)
            ...
            forwDurProbs(:,d,t) = tempForward(:,d);
        end
    ...
    for k=(numTimeSteps-1):-1:1,
        ...
        for d=1:min(numTimeSteps-k,maxDur)
            ...
            backDurProbs(:,d,t) = tempBackward(:,d);
        end
    ...
    fwbkNormedTransProbs = zeros(numAct,numAct, numTimeSteps-1);
    fwbkNormedDurProbs = zeros(numAct,maxDur, numTimeSteps-1);
    gamma = zeros(numAct, numTimeSteps);
    for k=1:numTimeSteps-1,
        tempfwbkTrans = zeros(numAct, numAct, maxDur);
        for d=1:min(k,maxDur)
            % Forward
            repBestProbs = repmatC(forwDurProbs(:,d,k),1,numAct);
            TransFwProb = probTrans(:,:,k) + repBestProbs;
            
            % Backward
            repprobObs = repmatC(cumBackObsProb(:,d,k+1),numAct,1) + repmatC(backDurProbs(:,d,k+1)',numAct,1);

            % Both put together
            tempfwbkTrans(:,:,d) = repprobObs+TransFwProb;
        end
        
        % Normalize 
        fwbkNormedTransProbs(:,:,k) = exp(logsum(tempfwbkTrans, 3)-logsum(logsum(tempfwbkTrans, 3)));
        fwbkNormedDurProbs(:,:,k) = exp(logsum(tempfwbkTrans, 2)-logsum(logsum(tempfwbkTrans, 2),3)); %reshape?
    end
    gamma(:,1:numTimeSteps-1) = reshape(sum(fwbkNormedTransProbs,2),numAct,numTimeSteps-1);
    gamma(:,numTimeSteps) =forwVar(:,numTimeSteps);
    
    fwbkProbsDays{n} = gamma;
    fwbkNormedTransProbsDays{n} = fwbkNormedTransProbs;
    fwbkNormedDurProbsDays{n} = fwbkNormedDurProbs;
end
